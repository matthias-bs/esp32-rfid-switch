...
The relay example wakes at the configured sleep interval, which defaults to five seconds (short interval to simplify testing; increase for actual application). Before the ESP32 enters deep or light sleep, the UHF reader is placed in manual idle mode with automatic idle entry disabled. The reader exits idle mode during the next initialization after deep sleep, or immediately after light sleep returns. The ESP32 sleep mode depends on whether the relay's configured GPIO is RTC-capable. An RTC-capable GPIO is connected to the ESP32 RTC GPIO subsystem and can retain its output level through deep sleep using GPIO hold; otherwise the example uses light sleep. Shelly mode uses deep sleep on every cycle, disconnects BLE, and exits reader idle mode during the next initialization.
